...
function ViewRequestSimple({request_fc, index}) {
    ...
    return (
        <Link key={index} to={"/factcheck/browse/" + request_fc.args.requestId} style={{textDecoration: "none"}}>
            <Card className="mb-3">
                {/* <Row>
                    <Col xs={3}>
                        <div className="thumbnail">
                            <img src={request_fc.args.thumbnail_url} alt="サムネイル" className="thumbnail img-fluid" />
                        </div>
                    </Col>
                    <Col xs={9}>
                        <Card.Body>
                            <Card.Title>{request_fc.args.title}</Card.Title>
                            <Card.Text>{request_fc.args.description}</Card.Text>
                            <Row>
                                <Col xs={4}>
